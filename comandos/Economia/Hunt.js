...
herramienta.establecer(`${user.id}.caña`, 0)
      ...
      let espadas = await herramienta.obtener(`${user.id}.espada`)
            let random = Math.floor(Math.random() * 1490) + 10
      const embed = new Discord.MessageEmbed()
      .setAuthor(`${user.tag}`, message.author.displayAvatarURL({ dynamic: true}))
      .setDescription("<:No:925269844197269534> No tienes una espada")
      .setColor("RED")
      const embe = new Discord.MessageEmbed()
      .setAuthor(`${user.tag}`, message.author.displayAvatarURL({ dynamic: true}))
      .setDescription(`Cazaste unos zombies y te pagaron ${random}<:Moneda:925044214901907487>`)
      .setColor("GREEN")
      if(cantidad > espadas){ return message.channel.send({ embeds: [embed] })} else{
