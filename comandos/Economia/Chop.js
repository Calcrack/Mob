...
herramienta.establecer(`${user.id}.caña`, 0)
      ...
      let hachas = await herramienta.obtener(`${user.id}.hacha`)
            let random = Math.floor(Math.random() * 2240) + 10
      const embed = new Discord.MessageEmbed()
      .setAuthor(`${user.tag}`, message.author.displayAvatarURL({ dynamic: true}))
      .setDescription("<:No:925269844197269534> No tienes una hacha")
      .setColor("RED")
      const embe = new Discord.MessageEmbed()
      .setAuthor(`${user.tag}`, message.author.displayAvatarURL({ dynamic: true}))
      .setDescription(`Talaste mucha madera y la vendiste por ${random}<:Moneda:925044214901907487>`)
      .setColor("GREEN")
      if(cantidad > hachas){ return message.channel.send({ embeds: [embed] })} else{
 ...
 cooldowns.set(message.author.id, Date.now() + 420000);
  setTimeout(() => cooldowns.delete(message.author.id), 420000);
